          ...
          {item.colors.map((item, index) => (
            <View 
              style={[styles.rowContainer, 
                index % 2 === 0 ? (availableColors.length > 0 ? styles.rowColorOnStock : styles.rowColorOutOfStock) : null ]} 
              key={`${index}-${item.color}`}
            >
              <Text style={styles.colorLabel2}>{item.color}</Text>
              <Text key={`${item.color}${index}`} style={styles.sizeText}>{item.stock}</Text>
            </View>
          ))}
        </Animated.View>
      )}
